...
def create_datbase(database_path: str):
    conn = lite.connect(database_path)
    with conn:
        cur = conn.cursor()
        cur.execute("drop table if exists words")
        ddl = "create table words (word text not null primary key, usage_cont int default 1 not null); "
        cur.execute(ddl)
...
def save_words_to_database(database_path: str, words_list: list):
    ...
    with conn:
        ...
        for word in words_list:
            ...
            if count > 0:
                sql = "update words set usage_cont = usage_cont + 1 where word = '" + word + "'"
            else:
